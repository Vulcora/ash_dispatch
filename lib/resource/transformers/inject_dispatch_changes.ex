defmodule AshDispatch.Resource.Transformers.InjectDispatchChanges do
  ...
  defp inject_into_action(dsl_state, action_name, event) do
    ...
    # 2. Otherwise, derive module name and check if it exists (was generated)
    # This enables the hybrid approach: DSL takes precedence, module provides fallback
  ...
  # Resolve event module using hybrid approach:
  # 1. If user explicitly set `module:` in DSL -> use that (override)
  # 2. Otherwise, derive module name and check if it exists (auto-connect)
  # 3. If derived module doesn't exist yet -> nil (first compile before codegen)
  #
  # This enables the workflow:
  # - First compile: module is nil, pure inline DSL mode
  # - Run `mix ash.codegen` to generate event modules
  # - Second compile: derived module found, auto-connected for hybrid mode
  defp resolve_event_module(event, resource, domain_name) do
    case event.module do
      # User explicitly set module (override)
      ...
      nil ->
        ...
        if Code.ensure_loaded?(derived_module) do
          ...
          # Module doesn't exist yet - will be generated by mix ash.codegen
          nil
        end
    end
  end
