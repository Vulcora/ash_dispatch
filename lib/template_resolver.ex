  ...
  * `{:error, reason}` - Rendering error
  ...
  defp render_from_priv_manifest(lookup_key, otp_app, transport, variant, format, assigns) do
    ...
    with {:ok, manifest} <- load_manifest(otp_app),
      ...
      render_template_content(template_content, assigns, format, otp_app: otp_app, transport: transport)
    else
  ...
  defp render_from_files(event_dir, transport, variant, format, assigns, add_templates_subdir, otp_app) do
