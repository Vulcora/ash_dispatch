defmodule AshDispatch.Resources.ManualTrigger.Helpers do
            ...
            Enum.map(channels, fn channel ->
              subject =
                if function_exported?(event_module, :subject, 2) do
                  try do
                    result = event_module.subject(context, channel)
                    Logger.debug("Subject computed: #{inspect(result)}")
                    result
                  rescue
                    e ->
                  ...
                  Logger.debug("No subject/2 function exported for #{inspect(event_module)}")
                  nil
                end
                ...
                if function_exported?(event_module, :prepare_template_assigns, 2) do
                  try do
                    ...
                    e ->
                      ...
                      {:error, "prepare_template_assigns failed: #{Exception.message(e)}. Check that required data is loaded (e.g., override required_resources/0)."}
                  end
                else
                  {:ok, context.data}
                end
        ...
        {:ok, event_config, resource} ->
          ...
          if Map.has_key?(context_data, id_field) do
            ...
            case load_resource(resource, resource_id, load_opts) do
              ...
              {:error, error} ->
                Logger.error("Failed to load #{inspect(resource)} with id #{inspect(resource_id)}: #{inspect(error)}")
                {:error, "Failed to load #{inspect(resource)} with id #{inspect(resource_id)}"}
            end
          else
            {:error, "Missing required field: #{id_field}. Provide the ID of the #{data_key} to preview."}
          end
  ...
  defp build_context_success(event_id, _event_module, base_data, context_data, actor) do
    Logger.debug("build_context_success - base_data keys: #{inspect(Map.keys(base_data))}")
    ...
    Logger.debug("build_context_success - merged_data keys: #{inspect(Map.keys(merged_data))}")
  ...
  defp load_data_from_required_resources(event_module, context_data) do
    ...
    if function_exported?(event_module, :resource, 0) do
      try do
        ...
        if Map.has_key?(context_data, id_field) do
          resource_id = context_data[id_field]
          Logger.debug("Loading #{inspect(resource)} with id #{inspect(resource_id)}, domain: #{inspect(domain)}, load: #{inspect(channel_load_opts)}")

          case load_resource_with_domain(resource, resource_id, channel_load_opts, domain) do
            {:ok, record} ->
              Logger.debug("Loaded record successfully, data_key: #{inspect(data_key)}")
              {:ok, %{data_key => record}}
          ...
          Logger.debug("Missing id_field #{inspect(id_field)} in context_data: #{inspect(context_data)}")
          {:error, "Missing required field: #{id_field}. Provide the ID of the #{data_key} to preview."}
        end
      rescue
        e ->
      ...
      if map_size(context_data) > 0 do
        ...
        {:error, "Event module has no resource/0 callback and no context data provided. " <>
  ...
  defp load_resource_with_domain(resource, id, load_opts, domain) do
    ...
    actual_domain = domain || (function_exported?(resource, :__domain__, 0) && resource.__domain__())
