  ...
  def handle_in("mark_all_notifications_read", _payload, socket) do
    ...
    case NotificationLoader.mark_all_as_read(user_id, actor: socket.assigns.current_user) do
      ...
      %Ash.BulkResult{errors: errors} ->
        {:reply, {:error, %{reason: "failed", details: inspect(errors)}}, socket}
    end
  end
...
```elixir
# Counters only
counters_state = ChannelState.build_counters(user_id)
# => %{"counters" => %{"pending_orders" => 5}}

# Notifications only
notifications_state = ChannelState.build_notifications(user_id, limit: 20)
# => %{"notifications" => [...]}
```
...
case NotificationLoader.mark_all_as_read(user_id, actor: current_user) do
  %Ash.BulkResult{status: :success} -> # Success
  %Ash.BulkResult{errors: errors} -> # Failed
end
```
...
defmodule MyAppWeb.UserChannel do
  ...
  def handle_in("mark_all_notifications_read", _payload, socket) do
    ...
    case NotificationLoader.mark_all_as_read(user_id, actor: socket.assigns.current_user) do
      ...
      %Ash.BulkResult{errors: errors} ->
        Logger.error("[UserChannel] Failed to mark all as read: #{inspect(errors)}")
        {:reply, {:error, %{reason: "failed"}}, socket}
    end
  end
...
- [Testing Events](testing-events.md) - Test your real-time features
