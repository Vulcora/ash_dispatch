  ...
  defp generate_gettext_catalog(events, otp_app) do
    ...
    if is_nil(gettext_backend) do
      ...
      if content_strings == [] do
        ...
        # Format the generated source to match the project formatter's canonical
        # output. A long dgettext line exceeds the line length, so `mix format`
        # wraps it on commit; without this the generator re-emits the un-wrapped
        # line on every run, leaving the file perpetually "modified" — which traps
        # the blessed-serve reconcile (it reads the formatting churn as operator
